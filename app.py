...
class LiteVizApp:
        ...
        try:
            ...
            if mol_config.get("enabled") and mol_config.get("molecule_col"):
                ...
                if mol_config["molecule_col"] not in hover_data:
                    hover_data.append(mol_config["molecule_col"])
            ...
            if not mol_config.get("enabled"):
                st.info("💡 Tip: Enable molecule visualization in the sidebar to see 3D structures when clicking plot points!")
    ...
    def _handle_plot_selection(
        ...
        try:
            ...
            molecule_name = selected_point["customdata"][0] if "customdata" in selected_point else None
    ...
    def _show_data_explorer(self, df: pd.DataFrame) -> None:
        ...
        if len(df) > max_display_rows:
            st.info(f"Showing first {max_display_rows:,} rows of {len(df):,} total rows")
